...
- 已加入 `fp4_activation_cache_lora_down_grad` fused CUDA 原型，避免 dense `x_hat` 中间张量；rank<=32 使用 `kVec=3,rVec=16`，rank<=256 使用 `kVec=3,rVec=32,threads=128`，rank>256 回落 `kVec=2,rVec=16`。tile sweep 中 rank32 的 4096 fused `dA` 从约 `0.391ms` 降到 `0.346ms`，约 `1.13x`；rank64 从约 `1.50ms` 降到 `0.99ms`，约 `1.52x`；rank128 从约 `3.34ms` 降到 `1.86ms`，约 `1.79x`；rank256 从约 `5.72ms` 降到 `3.68ms`，约 `1.55x`；候选记录见 [docs/fp4_kernel_research_notes.md](/home/wyj24/projects/nunchaku/extracted_nunchaku_core/docs/fp4_kernel_research_notes.md)。
...
- `native_fp4.fp4_activation_cache_lora_down_grad`
  - 直接从 native FP4 activation cache 计算 LoRA `dA` 的 fused CUDA 原型；rank<=32 使用 `kVec=3,rVec=16` fast path，rank<=256 使用 `kVec=3,rVec=32,threads=128` fast path，rank>256 回落 `kVec=2,rVec=16`；用于显存/近似训练消融，当前不建议默认开启
