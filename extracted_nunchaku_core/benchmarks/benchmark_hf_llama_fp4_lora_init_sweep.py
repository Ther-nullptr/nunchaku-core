...
LORA_INIT_CHOICES = ("zero", "gaussian", "residual_svd", "pissa")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run a same-batch HF/LLaMA FP4 LoRA init sweep. "
            "This compares zero/PiSSA/residual-SVD initialization under the same FP4 variant."
        )
    )
    ...
    parser.add_argument("--fp4-variant", choices=tuple(v for v in VALID_VARIANTS if v != "dense_lora"), default="fp4_balanced")
    parser.add_argument("--inits", nargs="+", choices=LORA_INIT_CHOICES, default=["zero", "pissa"])
    parser.add_argument("--no-dense-lora", action="store_true")
    ...
    if args.outlier_report and not os.path.exists(args.outlier_report):
        parser.error(f"--outlier-report does not exist: {args.outlier_report}")
    if args.sensitivity_report and not os.path.exists(args.sensitivity_report):
        parser.error(f"--sensitivity-report does not exist: {args.sensitivity_report}")
    return args
...
def add_relative_to_base_init(results: dict[str, Any], base_init: str = "zero") -> None:
    base = results["records"].get(base_init)
    if not base:
        return
    ...
    for init, record in results["records"].items():
        if init == base_init or init == "dense_lora":
            continue
        latency = train_step_latency(record)
        peak = peak_delta(record)
        rel_l2 = logits_rel_l2(record)
        record["relative_to_base_init"] = {
            "base_init": base_init,
            "speedup_vs_base_init": base_latency / latency,
            "latency_ratio_vs_base_init": latency / base_latency,
            "peak_delta_ratio_vs_base_init": None if base_peak == 0 else peak / base_peak,
            "initial_loss_delta_vs_base_init": float(record["initial_loss"]) - base_loss,
            "logits_rel_l2_ratio_vs_base_init": None
...
def main() -> None:
    ...
    inits = list(dict.fromkeys(args.inits))
    ...
    results: dict[str, Any] = {
        "experiment": "hf_causal_lm_fp4_lora_init_sweep",
    ...
    out_path = os.path.join(args.results_dir, f"hf_llama_fp4_lora_init_sweep_{stamp}.json")
    latest_path = os.path.join(args.results_dir, "latest_hf_llama_fp4_lora_init_sweep.json")
