...
目标：优化 `fp4_activation_cache_lora_down_grad` 的 rank32/rank64/rank128/rank256 常见 LoRA 形状，降低 `fp4_activation_cache_d_lora_down=True` 显存模式的 backward 开销。
...
- 对 rank256 以外的更高 LoRA rank 仍需继续做 tile sweep；当前 rank > 256 保持旧 `kVec=2,rVec=16`，避免无证据推广。
