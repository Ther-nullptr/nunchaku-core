...
`fp4_lora_sequence_finetune_config(...)` 是 shape-aware wrapper：给定 `flattened_rows` 或 `batch_size * seq_len`，在短序列/长序列之间选择不同 preset。默认短序列走 `balanced`，长序列走 `memory_saving`，并把 `fp4_activation_cache_min_rows` 设为同一个阈值；这对应 FlashInfer contest solution 中的 shape dispatcher 思路，避免 decode/短序列误进近似显存模式，同时让长 prefill/batch 能用 FP4 activation-cache 降 saved activation。真实 HF benchmark 可用 `--variants fp4_auto_seq_policy` 记录 `sequence_policy` decision。
...
`benchmark_hf_llama_fp4_lora_finetuning.py` 会在真实 HF/LLaMA + WikiText-2 结果中额外输出 `variant_summary`，按 train-step latency、峰值显存、accuracy metric 和 Pareto frontier 汇总 FP4 variants。若有 dense LoRA baseline，accuracy metric 使用 `initial_logits_rel_l2_vs_dense_lora`；否则回落到同 batch `initial_loss`。该脚本支持 `--grad-accum-steps N`：一次被计时的 `train_step_with_optimizer` 包含 `N` 个 micro-batch 的 `forward + backward` 和一次 optimizer step，loss 按 `1/N` 缩放；`tokens_per_second` 按 `batch_size * seq_len * N` 计算，`micro_steps_per_second` 用于观察 micro-batch 级吞吐。长实验应通过 `scripts/run_tmux_experiment.py` 启动，避免 SSH 中断并保留 `runs/tmux/` 日志。
...
- `init`：
  ...
  - `pissa`：用 `W0` 的 principal SVD 初始化 LoRA，并把 FP4 backbone 量化为 `W0 - BA`；这是 QPiSSA 风格初始化，避免主成分在 backbone 和 LoRA 中双计。
...
- trainable `init="residual_svd"` 和 `init="pissa"` 使用 dense LoRA dX 时 correctness 通过；若同时打开 `fuse_lora_dx=True`，BF16 下较大的 low-rank factors 会把 fused LoRA dX 近似误差放大到约 `2e-3`，因此推荐把 residual_svd 作为 frozen residual branch，task LoRA 仍 zero-init；PiSSA 更适合初始化/收敛精度消融。
...
初始化消融，RTX 5090 BF16，`benchmark_fp4_lora_initialization.py --m 2048 --in-features 2048 --out-features 2048 --rank 32 --warmup 5 --iters 10`。脚本同时输出 `fp4_pissa_lora_full/lowrank`，用于比较 QPiSSA-style 初始化相对 zero/frozen residual_svd 的初始误差和构造开销：
